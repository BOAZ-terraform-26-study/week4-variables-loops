...
| **메타 인자 (meta-argument)** | 리소스 타입이 무엇이든 `resource` · `data` · `module` 블록에 쓸 수 있는 인자. `count` · `for_each` · `depends_on` · `lifecycle` · `provider`. `output` 이나 `variable` 에는 쓰지 못합니다 | - |
...
블록을 열면 곧바로 막히는 데가 있습니다. `t3.micro`는 글자 하나로 끝나지만 서브넷 목록은 그렇지 않습니다. 서브넷 하나를 만들려면 CIDR 과 AZ 가 둘 다 필요하고, 서브넷이 두 개면 그 짝이 두 벌 있어야 합니다. 받아야 하는 값이 이렇게 생겼습니다.
...
Part 1에서 타입을 정했습니다. 그런데 타입은 값의 생김새만 봅니다. 안에 적힌 내용이 맞는지는 보지 않습니다.

`type = string` 이라고 적어두면 아래 셋이 전부 통과합니다.

```
"10.0.1.0/24"        맞는 값
"10.0.1.0/240"       /240 이라는 대역은 없습니다
"10.0.1"             CIDR 표기가 아닙니다
```
...
Part 1의 1번에서 하드코딩이 두 가지를 막는다고 했습니다. 값을 바꾸려면 리소스 정의를 열어야 하는 것, 그리고 같은 리소스를 하나 더 만들려면 블록을 통째로 복사해야 하는 것입니다. 앞의 것을 Part 3까지 풀었고 여기서 뒤의 것을 풉니다.
...
이러면 서브넷이 다섯 개일 때 블록이 다섯 개가 되고, 공통으로 고칠 것이 생기면 다섯 군데를 고쳐야 합니다. 대신 블록 하나에 "이 블록을 몇 벌 만들어라"를 붙이는 방법이 있습니다. 그런 인자를 메타 인자라고 부르고, 리소스 타입이 무엇이든 `resource` · `data` · `module` 블록에 붙일 수 있습니다. `output`이나 `variable`에는 붙지 않습니다.
